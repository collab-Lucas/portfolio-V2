...
function joinWithSlash(start, end) {
  ...
  return end.startsWith("/") ? start + end : "".concat(start, "/").concat(end);
}
...
function normalizeQueryParams(params) {
  return params && params[0] !== "?" ? "?".concat(params) : params;
}
...
function assertTemplate(templateRef, property) {
  if (templateRef && !templateRef.createEmbeddedView) {
    throw new RuntimeError(2020, (typeof ngDevMode === "undefined" || ngDevMode) && "".concat(property, " must be a TemplateRef, but received '").concat(stringify(templateRef), "'."));
  }
}
...
function createCloudflareUrl(path, config) {
  ...
  if (config.isPlaceholder) {
    params += ",quality=".concat(PLACEHOLDER_QUALITY);
  }
...
function createCloudinaryUrl(path, config) {
  ...
  let params = "f_auto,".concat(quality);
  if (config.width) {
...
function createImagekitUrl(path, config) {
  ...
  const params = [];
  if (width) {
    params.push("w-".concat(width));
  }
  if (config.isPlaceholder) {
    params.push("q-".concat(PLACEHOLDER_QUALITY));
  }
  const urlSegments = params.length ? [path, "tr:".concat(params.join(",")), src] : [path, src];
...
var _PreloadLinkCreator = class _PreloadLinkCreator {
  constructor() {
    __publicField(this, "preloadedImages", inject(PRELOADED_IMAGES));
    __publicField(this, "document", inject(DOCUMENT));
  ...
  createPreloadLinkTag(renderer, src, srcset, sizes) {
    if (ngDevMode && !this.errorShown && this.preloadedImages.size >= DEFAULT_PRELOADED_IMAGES_LIMIT) {
      ...
      console.warn(formatRuntimeError(2961, "The `NgOptimizedImage` directive has detected that more than " + "".concat(DEFAULT_PRELOADED_IMAGES_LIMIT, " images were marked as priority. ") + 'This might negatively affect an overall performance of the page. To fix this, remove the "priority" attribute from images with less priority.'));
    }
...
var _NgOptimizedImage = class _NgOptimizedImage {
  constructor() {
    __publicField(this, "imageLoader", inject(IMAGE_LOADER));
    __publicField(this, "config", processConfig(inject(IMAGE_CONFIG)));
    __publicField(this, "renderer", inject(Renderer2));
    __publicField(this, "imgElement", inject(ElementRef).nativeElement);
...
function postInitInputChangeError(dir, inputName) {
  ...
  return new RuntimeError(2953, "".concat(imgDirectiveDetails(dir.ngSrc), " `").concat(inputName, "` was updated after initialization. ") + "The NgOptimizedImage directive will not react to this input change. ".concat(reason, " ") + "To fix this, either switch `".concat(inputName, "` to a static value ") + "or wrap the image element in an @if that is gated on the necessary value.");
}
  ...
  return __async(this, null, function* () {
    if (IMGS_WITH_PRIORITY_ATTR_COUNT === 0) {
      ...
      if (IMGS_WITH_PRIORITY_ATTR_COUNT > PRIORITY_COUNT_THRESHOLD) {
        console.warn(formatRuntimeError(2966, 'NgOptimizedImage: The "priority" attribute is set to true more than '.concat(PRIORITY_COUNT_THRESHOLD, " times (").concat(IMGS_WITH_PRIORITY_ATTR_COUNT, " times). ") + 'Marking too many images as "high" priority can hurt your application\'s LCP (https://web.dev/lcp). "Priority" should only be set on the image expected to be the page\'s LCP element.'));
      }
    } else {
      IMGS_WITH_PRIORITY_ATTR_COUNT++;
    }
...
function assertPlaceholderDimensions(dir, imgElement) {
  ...
  if (renderedWidth > PLACEHOLDER_DIMENSION_LIMIT || renderedHeight > PLACEHOLDER_DIMENSION_LIMIT) {
    console.warn(formatRuntimeError(2967, "".concat(imgDirectiveDetails(dir.ngSrc), " it uses a placeholder image, but at least one ") + "of the dimensions attribute (height or width) exceeds the limit of ".concat(PLACEHOLDER_DIMENSION_LIMIT, "px. ") + "To fix this, use a smaller image as a placeholder."));
  }
}
...
@angular/common/fesm2022/common.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-6ZXMLLRP.js.map
