...
var ACCEPT_HEADER_VALUE = "".concat(JSON_CONTENT_TYPE, ", ").concat(TEXT_CONTENT_TYPE, ", */*");
  ...
  request(first, url, options = {}) {
    ...
    switch (options.observe || "body") {
      ...
      case "response":
        return res$;
      default:
        throw new RuntimeError(2809, ngDevMode && "Unreachable: unhandled observe type ".concat(options.observe, "}"));
    }
  }
  /**
...
var _FetchBackend = class _FetchBackend {
  constructor() {
    ...
    __publicField(this, "fetchImpl", (_b = (_a = inject(FetchFactory, {
      optional: true
...
var XSSI_PREFIX = /^\)\]\}',?\n/;
var X_REQUEST_URL_REGEXP = RegExp("^".concat(X_REQUEST_URL_HEADER, ":"), "m");
...
var HttpResourceImpl = class extends ResourceImpl {
  constructor(injector, request, defaultValue, parse, equal) {
    ...
    __publicField(this, "_headers", linkedSignal({
      source: this.extRequest,
      computation: () => void 0
    }));
    __publicField(this, "_progress", linkedSignal({
      source: this.extRequest,
      computation: () => void 0
    }));
    __publicField(this, "_statusCode", linkedSignal({
      source: this.extRequest,
      computation: () => void 0
    }));
    __publicField(this, "headers", computed(() => this.status() === ResourceStatus.Resolved || this.status() === ResourceStatus.Error ? this._headers() : void 0));
...
function sortAndConcatParams(params) {
  return [...params.keys()].sort().map((k) => "".concat(k, "=").concat(params.getAll(k))).join("&");
}
  ...
  supports(eventName) {
    ...
    if (!window.Hammer && !this.loader) {
      if (typeof ngDevMode === "undefined" || ngDevMode) {
        ...
        _console.warn('The "'.concat(eventName, '" event cannot be bound because Hammer.JS is not ') + "loaded and no custom loader has been specified.");
      }
      return false;
    }
    return true;
  ...
  addEventListener(element, eventName, handler) {
    ...
    if (!window.Hammer && this.loader) {
      ...
      zone.runOutsideAngular(() => this._loaderPromise.then(() => {
        ...
        if (typeof ngDevMode === "undefined" || ngDevMode) {
          const _console = this._injector.get(Console);
          _console.warn('The "'.concat(eventName, '" event cannot be bound because the custom ') + "Hammer.JS loader failed.");
        }
        deregister = () => {
        };
      }));
